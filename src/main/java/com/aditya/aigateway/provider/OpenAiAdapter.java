...
public class OpenAiAdapter implements AiProvider {
    ...
    @Autowired
    public OpenAiAdapter(WebClient.Builder webClientBuilder, ModelRoutingConfig config) {
    ...
    public boolean supports(String modelName) {
        return modelName != null && modelName.startsWith("gpt");
    }
    ...
    public GatewayResponse call(GatewayRequest request, String modelName) {
        ...
        String apiKey = config.getProviders().get("openai").getApiKey();
        String baseUrl = config.getProviders().get("openai").getBaseUrl();

        log.debug("Calling OpenAI | model={}", modelName);
        ...
        Map response = webClient.post()
                ...
                .timeout(Duration.ofSeconds(config.getProviders().get("openai").getTimeoutSeconds()))
                .block();
